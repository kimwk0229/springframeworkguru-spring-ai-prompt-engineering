...
public class SummarizingPromptsTests extends BaseTestClass {
    ...
    @Test
    // 단일 리뷰에서 설명을 생성하는 테스트 메서드
    void testCreateDescriptionFromReview() {
        var promptTemplate = new PromptTemplate(reviewPrompt,
    ...
    @Test
    // 세 개의 리뷰에서 설명을 생성하는 테스트 메서드
    void testCreateDescriptionFrom3Reviews() {
        var promptTemplate = new PromptTemplate(reviewPrompt3,
                Map.of("review", review1, "review2", review2, "review3", review3));

        System.out.println(chatModel.call(promptTemplate.create()).getResult().getOutput().getContent());
    }

    // 세 개의 리뷰에서 요약을 추출하기 위한 프롬프트 템플릿 정의 (reviewPrompt3와 동일하지만 이름이 다름)
    ...
    @Test
    // 세 개의 리뷰에서 설명을 추출하는 테스트 메서드
    void testCreateDescriptionFrom3ReviewsExtract() {
        var promptTemplate = new PromptTemplate(reviewPrompt4,
                Map.of("review", review1, "review2", review2, "review3", review3));
